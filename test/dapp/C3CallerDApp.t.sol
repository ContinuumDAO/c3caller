...
contract C3CallerDAppTest is Helpers {
    ...
    function setUp() public override virtual {
        ...
        mockC3CallerDAppID = _initDAppConfig(gov, mockDAppKey, address(usdc), mockMetadata);
        mockC3CallerDApp = new MockC3CallerDApp(address(c3caller), mockC3CallerDAppID);

        vm.startPrank(gov);
        usdc.approve(address(dappManager), type(uint256).max);
        dappManager.deposit(mockC3CallerDAppID, address(usdc), 100 * 10 ** usdc.decimals());
        dappManager.setDAppAddr(mockC3CallerDAppID, address(mockC3CallerDApp), true);
        vm.stopPrank();
    ...
    function test_State() public view {
        ...
        assertEq(_c3caller, address(c3caller));
    }
    ...
    function test_C3Call_Success() public {
        ...
        bytes memory data = abi.encodeWithSelector(MockC3CallerDApp.mockC3Executable.selector, message);
        bytes32 uuid = uuidKeeper.calcCallerUUID(address(c3caller), mockC3CallerDAppID, to.toHexString(), toChainID, data);
